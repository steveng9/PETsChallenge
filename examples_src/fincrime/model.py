...
class SwiftModel:
    ...
    def fit(self, train: pd.DataFrame):
        ...
        X_train = encoder_df[SWIFT_COLS + list(range(BIN_GRANULARITY * 2 + 3))].values
    ...
    def predict(self, test: pd.DataFrame):
        ...
        X_test = encoder_df[SWIFT_COLS + list(range(BIN_GRANULARITY * 2 + 3))].values
...
def create_bins(swift_df: pd.DataFrame):
    ...
    split = mean(swift_df[swift_df["Label"] == 0]["InterimTime"], epsilon=0.01, bounds=(min1_, 0))
    fuzzy1 = 10_000
    fuzzy2 = 10_000

    min2_ = -1 * 86400
    begin1, end1 = percentile(swift_df[swift_df["Label"] == 0][swift_df["InterimTime"] > split]["InterimTime"],[0, 100], bounds=(min2_, 0), epsilon=0.3)
    begin1 -= fuzzy1
    end1 += fuzzy1

    begin2, end2 = percentile(swift_df[swift_df["Label"] == 0][swift_df["InterimTime"] < split]["InterimTime"],[0, 100], bounds=(min1_, (min1_ - min2_)), epsilon=0.3)
    begin2 -= fuzzy2
    end2 += fuzzy2
