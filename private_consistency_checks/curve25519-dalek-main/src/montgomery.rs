...
use crate::{constants::{APLUS2_OVER_FOUR, MONTGOMERY_A, MONTGOMERY_A_NEG}, field::FieldElement51};
...
// const Zu: FieldElement51 = FieldElement51([1066188786548365, 1781982046572228, 36570682222399, 269194373326530, 720847714518980]);

// /// Encoding according to https://elligator.org/formulas
// pub fn elligator_encode(r: &FieldElement) -> MontgomeryPoint {
//     let u = r.square();
//     let t1 = &u * &TWO;
...
//     let t1 = &t2 * &v;
//     let (s, t1) = FieldElement::invsqrt(&(&t3 * &t1));
//     let u = &u * &Zu;
//     let u = FieldElement::conditional_select(&u, &FieldElement::ONE, s);
//     let t1 = t1.square();
//     let u = &u * &MONTGOMERY_A_NEG;
//     let u = &u * &t3;
//     let u = &u * &t2;
//     let u = &u * &t1;
...
// const TWO: FieldElement51 = FieldElement51([2, 0, 0, 0, 0]);

const MINUS_TWO: FieldElement51 = FieldElement51([2251799813685227,
    2251799813685247,
    2251799813685247,
    2251799813685247,
    2251799813685247]);

/// Computes the inverse Elligator2 map, returns None if there is no representative. (see: https://elligator.org/formulas)
pub fn elligator_decode(point: &MontgomeryPoint, v_negative: Choice) -> Option<FieldElement> {
    let mut u = FieldElement::from_bytes(point.as_bytes());
    let t = &u + &MONTGOMERY_A;
    let mut r = &MINUS_TWO * &u;
    r *= &t;
    //let (s, mut r) = FieldElement::sqrt_ratio_i(&u, &FieldElement::ONE);
    let (s, mut r) = FieldElement::invsqrt(&r);
    if s.unwrap_u8() == 0 {
        return None;
    }

    u = FieldElement::conditional_select(&u, &t, v_negative);

    r *= &u;
    let mut t = r.clone();
    t.negate();

    Some(FieldElement::conditional_select(&r, &t, r.is_negative()))
}
...
mod test {
    ...
    #[test]
    fn elligator_inverse_identity_notexists() {
        let point = MontgomeryPoint::identity();
        let representative = elligator_decode(&point, Choice::from(0));
        assert!(representative.is_none());
    }

    #[test]
    fn elligator_inverse_hardcoded() {
        let bytes: Vec<u8> = (0u8..32u8).collect();
        let bits_in: [u8; 32] = (&bytes[..]).try_into().expect("Range invariant broken");
        ...
        assert_eq!(elligator_decode(&eg, Choice::from(0)).unwrap(), fe);

        assert_eq!(elligator_encode(&elligator_decode(&eg, Choice::from(0)).unwrap()), eg);
        assert_eq!(elligator_encode(&elligator_decode(&eg, Choice::from(1)).unwrap()), eg);
    }
